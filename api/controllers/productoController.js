...
function index(req, res) {
    productoModel.find({})
      .then(products => {
        if (products.length) return res.status(200).send({products} || { message: 'Muestra Todos Los Productos'});
  ...
  function show(req, res) {
    ...
    // console.log('[------>] Buscar Show' + req.body.products);
  ...
  function update(req, res) {
      ...
      product.save().then(product => res.status(200).send({message: 'Se Actualizó: ', product}))
      .catch(error => res.status(500).send({error}));
  } 
  ...
  function remove(req, res) {
      if(req.body.error) return res.status(500).send({error});
      if(!req.body.products) return res.status(404).send({message: 'Producto No Encontrado'});
      req.body.products[0].remove().then(product => res.status(200).send({message: 'Se Eliminó', product}))
      .catch(error => res.status(500).send({error}));
  }
  ...
  function buscar(req, res, next) {
    ...
    productoModel.find(query).then(products => {
